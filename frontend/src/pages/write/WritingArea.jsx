    ...
    <div className="flex flex-col gap-4">
      ...
      {submitted && (
        <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-[20px] p-4 text-green-700 animate-in fade-in slide-in-from-top-2">
          <CheckCircle size={18} />
          <span className="text-sm font-semibold">Submission posted successfully! Check the Correct section to see it.</span>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-2 min-h-[24px]">
        <div className="flex items-center gap-2">
          {isTooLong && (
            <div className="flex items-center gap-1.5 text-red-500 animate-in fade-in slide-in-from-left-2">
              <AlertCircle size={14} />
              <span className="text-[12px] font-bold">Word limit exceeded!</span>
            </div>
          )}
        ...
        <div className={`text-[13px] font-bold transition-colors ${isTooLong ? "text-red-500" : "text-gray-400"}`}>
          {wordCount} <span className="font-medium text-gray-300">/ 300 words</span>
        </div>
      </div>
