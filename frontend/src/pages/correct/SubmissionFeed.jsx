...
export default function SubmissionFeed({ activeFilters }) {
  const { submissions } = useSubmissionStore();

  const filtered = submissions.filter(item => {
    const langMatch = activeFilters.language === "All" || item.language === activeFilters.language;
    const levelMatch = activeFilters.level === "All" || item.level === activeFilters.level;
