...
export default function Correct() {
  ...
  console.log(id);
  const { submissions, addReview } = useSubmissionStore();

  const submission =
    location.state?.submission ||
    submissions.find((item) => String(item.id) === id);
        ...
        <div className='lg:col-span-4 space-y-4'>
          ...
          <div className='bg-[#EEF2FF] rounded-[32px] p-8 border border-indigo-50'>
            ...
            <p className='text-[#1A1A1A] leading-relaxed text-[15px]'>
              {submission?.text ||
                'Lorem ipsum dolor sit amet consectetur. Amet est urna elementum quam risus nisl diam.'}
            </p>
          </div>
        </div>
